...
<body >
	...
	<div style="padding: 10px;" class="form-inline">
		  ...
		  <!-- <div class="panel-body"> -->
			  <table class="table" style="margin:0">
					...
					<tr style="background-color: #f9f9f9;">
						<td  width=15% style="text-align: center;">服务队名称：</td>
						<td width=35%  id="name" >${serTeam.name}</td>
						<td width=15% style="text-align: center;">负责人：</td>
						<td width=35% >${serTeam.principal}</td>
					</tr>
					<tr style="background-color: #f9f9f9;">
						<td style="text-align: center;">服务队描述：</td>
						<td id="" colspan="3">${serTeam.summary}</td>
		...
		<div  style="padding-top: 5px;">
            ...
            <a class="btn btn-success btn-sm" onclick="addTeamUser(${serTeam.id})" style="margin-bottom: 10px;">
                <i class="fa fa-plus">添加用户</i>
            </a>
          
            <a class="btn btn-danger btn-sm" onclick="delTeamUser()" id="del" style="margin-bottom: 10px;">
            	<i class="fa fa-trash-o">移除用户</i>
            </a>
	                    
		    <form name="searchRoleUserForm" class="form-inline" id="searchRoleUserForm" action="${webroot}serTeam/serTeamUsers.do" style="margin-bottom: 10px;float: right;" method="post" onsubmit="return false">
				<input type="hidden" name="teamId" id="teamId" value="${serTeam.id}">
...
<script type="text/javascript">
	...
	function addTeamUser(id){
		...
		var index=layer.open({
		  ...
		  area: ['800px', '640px'],
// 		  scrollbar: true,
		  content: "${webroot}serviceteam/addTeamUser.html?tId="+id,
		  btn : [ '<i class="fa fa-floppy-o"></i>确定','<i class="fa fa-remove"></i>取消'], 
		  yes :function(index, layero) {
				var iframeWin = top.window[layero.find('iframe')[0]['name']]; 
				var result=iframeWin.addUsersUnderTeam();
	...
	*/
	...
	function delTeamUser(){
		...
		if(!ids){
			...
			layer.confirm('确定要从服务队中移除选中的用户吗？', {icon: 3, title:'系统提示',
			  btn: ['确定','取消']
			}, function(){
				var url="${webroot}serviceteam/delTeamUser.do";
				params={"ids":ids,"teamId":teamId};
				postData(url, params, true, delTeamUserResult); 
			});
		}
	}
	...
	**回调函数
	*/
	function delTeamUserResult(data){
		var laytpl = layui.laytpl;
		var layer = layui.layer;
		var message=data.message;
