...
<body class="v-page-body">
	<div class="page-container">
		<div class="panel">
			<form id="editMenuForm" action="${webroot}menu/editMenu.do" method="post">
				<div class="row">
				...
				<table class="table table-bordered dataTable no-footer">
					<tr>
						...
						<td class="col-xs-10">
							<input type="text" class="form-control" name="menuName" placeholder="菜单名称(2-30位的字母)"  value="${menu.menuName}"
								data-validation="custom" data-validation-regexp="^[a-zA-Z_]{2,30}$"
		                   		data-validation-error-msg-custom="此项必须为2-30位的字母"
					...
					<tr>
						...
						<td class="col-xs-10">
							<input type="text" class="form-control" name="menuDesc" placeholder="菜单名称(2-50位的汉字与字母)"  value="${menu.menuDesc}"
					...
					<tr>
						...
						<input type="radio" ${menu.menuType==1?"checked='checked'":""} value="1" name="menuType">按钮
                  		</td>
					</tr>
					<tr>
						<td class="col-xs-2"><label for="inputEmail1">菜单图标</label></td>
						<td class="col-xs-10">
							<input type="text" class="form-control" name="icon" placeholder="菜单图标"  value="${menu.icon}" />
                  		</td>
					</tr>
					<tr>
						<td class="col-xs-2"><label for="inputEmail1">菜单URL</label></td>
						<td class="col-xs-10">
							<input type="text" class="form-control" name="url" placeholder="菜单URL"  value="${menu.url}" />
                  		</td>
					</tr>
					...
					<tr>
						...
						<td class="col-xs-10">
							<input type="text" class="form-control" name="orderNo" placeholder="菜单排序号"  value="${menu.orderNo}" />
					...
					<tr>
						...
						<td class="col-xs-10">
							...
							<input type="radio" ${menu.isShow==0?"checked='checked'":""} value="0" name="isShow">不可见
                  		</td>
					</tr>
				</table>
				</div>
	...
	$(document).ready(function(){
		postData("./loadMenu.do", {}, true, function (result){
			...
			if(success){
				...
				var node =zTree.getNodeByParam("id", "${menu.parentId}", null);
	...
	$('#sub').on('click', function() {
		errors =[];
		if( !$('#editMenuForm').isValid({},conf, true)) {
			...
			clearValid("editMenuForm");
			return false;
	   	}else {
	   		clearValid("editMenuForm");
	   		$("#sub").prop("disabled",true);
			postFormData("editMenuForm",true,editResult);
	    } 
	});
