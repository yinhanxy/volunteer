...
<body >
	<div style="margin-left:15px;margin-top:10px;line-height: 26px!important;"><a href="javascript:loadPage();">用户组列表</a></div>
	<div class="col-sm-1" id="areaTree" style="border:1px solid #eee;margin-left:15px;height: 500px;width: 200px;float:left;overflow-x:auto;overflow-y:auto;">
		<div id="tree" class="ztree" style="margin-top:10px;"><!--  -->
		
			
		</div>
	</div>
	<div style="padding:0px 20px;width: 710px;float:left;" class="form-inline">
		
		<div  style="padding-top: 10px;float:right;">
		                    
		    <form class="form-inline" id="searchUsersForm" action="${webroot}role/usersRequredRole.do" style="margin-bottom: 10px;" onsubmit="return false">
		...
		<script id="templateData" type="text/html">
							{{#  layui.each(d.list, function(index, item){ }}	
			                            ...
			                            <td width="40" style="text-align:center">
											<input type="checkbox" class="ace" value="{{item.id}}" {{item.roleIdList==""?"":"name='isOwn'"}}>
										</td>
	...
	function addUsersUnderRole(){
		...
		if(!ids){
			layer.msg('请选择要分配的用户！',{time:1000});
				...
				var url="${webroot}role/addRoleUsers.do";
				params={"ids":ids,"roleId":roleId};
				postData(url, params, true,userRoleResult); 
