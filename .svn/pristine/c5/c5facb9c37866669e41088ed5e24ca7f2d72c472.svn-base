...
<body >
	<div style="padding: 20px;" class="form-inline" >
		...
		<div  style="padding-top: 10px;float:right;">
		    ...
		    <form class="form-inline" id="searchUsersForm" action="${webroot}org/usersRequredOrg.do" style="margin-bottom: 10px;" onsubmit="return false">
		...
		<script id="templateData" type="text/html">
							{{#  layui.each(d.list, function(index, item){ }}	
			                            ...
			                            <td width="40" style="text-align:center">
											<input type="checkbox" class="ace" value="{{item.id}}" {{item.orgId==""?"":"name='isOwn'"}}>
										</td>
	...
	<div class="v-form-footer">
	  			<button type="button" id="sub" class="btn btn-sm btn-success" onclick="addUsersUnderOrg()"><i class="fa fa-floppy-o">  保存</i></button>
	   			<button type="button" class="btn btn-sm" onclick="closeFrame()"><i class="fa fa-remove">  取消</i></button>
	</div>
...
<script type="text/javascript">
	...
	function addUsersUnderOrg(){
		...
		if(!ids){
			...
			var url="${webroot}org/addOrgUsers.do";
			params={"ids":ids,"orgId":orgId};
			postData(url, params, true,userOrgResult); 
	...
	function searchResult(data){
		...
		if(data.success){
    		...
    		if(list && list.length > 0){
        		...
        		var pageHtml = createOrgUserPage(page, "searchUsersForm");
