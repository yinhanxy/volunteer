...
public class RoleValidator extends BaseTest{
	...
	public void validate(){
		...
		ValidatorUtil.validate(role, Groups.Add.class);
	...
	public void validatePropertyByGroup(){
		Role role = new Role();
		role.setRoleName("");
        List<ValidateMessage> errors = ValidatorUtil.validateProperty(role,"roleName",Groups.Add.class);
		if(errors != null && errors.size() > 0){
			for(ValidateMessage error : errors){
				System.out.println(error);
			}
