...
<body class="v-page-body">
		<div class="panel" style="margin-bottom: 0;padding-top: 10px;width: 100%;position: absolute;padding-right: 10px;padding-left: 10px;">	
				<form id="editActivityForm" action="${webroot}activity/editActivity.do">
					...
					<table class="table table-bordered dataTable no-footer">
						<tr class="row">
							...
							<td class="col-xs-4">
								<label class="radio-inline">
							  	<input type="radio" name="type" value="1" style="margin-top: 9px;" ${activity.type==1?"checked":""}/>短期
								</label>
								<label class="radio-inline">
									<input type="radio" name="type" value="2" style="margin-top: 9px;" ${activity.type==2?"checked":""}/>长期
								</label>
                   			</td>
						</tr>
						<tr class="row">
							...
							<td colspan="3">
								...
								data-validation="required" data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1">${activity.requirements}</textarea>
                   			</td>
						</tr>
						<tr class="row">
						...
						<tr class="row">
							...
							<td class="col-xs-4">
									<input type="text"  class="form-control" name="hours"   placeholder="活动服务时长(小时)" data-validation="required custom"
						...
						<tr class="row">
							...
							<td class="col-xs-4">
									<input type="text"  class="form-control" name="recruitSt" id="recruitSt" readonly="readonly" placeholder="招募开始时间" data-validation="required"
									value="<fmt:formatDate value='${activity.recruitSt}' pattern='yyyy-MM-dd HH:mm:ss'/>"
									data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1"/>
                   			</td>
							...
							<td class="col-xs-4">
									...
									value="<fmt:formatDate value='${activity.recruitEt}' pattern='yyyy-MM-dd HH:mm:ss'/>"
								data-validation-error-msg-required="此选项为必填" 
								data-validation-error-msg-container="#nError6"/>
                   			</td>
						</tr>
						<tr class="row">
							...
							<td class="col-xs-4">
									...
									value="<fmt:formatDate value='${activity.activitySt}' pattern='yyyy-MM-dd HH:mm:ss'/>"
									data-validation="required" data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1"/>
                   			</td>
							...
							<td class="col-xs-4">
									...
									value="<fmt:formatDate value='${activity.activityEt}' pattern='yyyy-MM-dd HH:mm:ss'/>"
								data-validation-error-msg-required="此选项为必填" 
								data-validation-error-msg-container="#nError6"/>
                   			</td>
						</tr>
						<tr class="row">
							...
							<td><select class="form-control" name="recruitRange" data-validation="required"
						    	data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1">
		           					...
		           					<option value="1" ${activity.recruitRange==1?"selected":""}>公开招募</option>
		           					<option value="2" ${activity.recruitRange==2?"selected":""}>只限本服务队</option>
	           					</select>
                   			</td>
						</tr>
						<tr class="row">
							...
							<td colspan="3">
								...
								data-validation="required" data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1">${activity.summary}</textarea>
                   			</td>
						</tr>
					</table>
			    ...
			    <div class="v-form-footer">
		  			...
		  			<shiro:hasPermission name="/activity/submitActivity.do">
		  				<button type="button" id="saveAndSubmit" class="btn btn-sm btn-success" ><i class="fa fa-save"></i>保存并提交</button>
					</shiro:hasPermission>
					<shiro:hasPermission name="/activity/publication.do">
						<button type="button" id="saveAndPublish" class="btn btn-sm btn-success" ><i class="fa fa-save"></i>保存并发布</button>
					</shiro:hasPermission>
	...
	layui.use(['jquery','layer','laytpl'], function(){
		$("#editActivityForm input,select").on("change",function(){
	 ...
	 $.formUtils.addValidator({
		...
		$('#save').on('click', function() {
			...
			if( !$('#editActivityForm').isValid({},conf, true)) {
				isForm=false;
				...
				postFormData("editActivityForm",true,saveActivity);
		    }
	...
	function showActivityClass(){
		...
		params={};
		postData(url, params, true, showActivityClassResult); 
	}
