...
<body class="v-page-body">
	<div class="page-container">
		<div class="panel">	
				<form id="editAreaForm" action="${webroot}area/saveArea.do">
					<div class="row">
					<table class="table table-bordered dataTable no-footer">
						<tr>
							...
							<td class="col-xs-4">
								<div class="input-group" onclick="selectArea()">
									<input type="hidden" name="parentId"  id="parentId" value="${area.pId}"/>
									<input type="hidden" name="id"  value="${area.id }"/>
									<input type="text"  class="form-control" name="parentName"  id="parentName" readonly="readonly"  value="${area.parentName}" 
									/>
						...
						<tr>
							...
							<td>
								<input type="text" class="form-control" name="name" value="${area.name}" placeholder="区域名称(至少由两个汉字组成)" data-validation="required custom" data-validation-regexp="^[\u4e00-\u9fa5]{2,50}$"
						...
						<tr>
							...
							<td><input name="code" class="form-control" value="${area.code}"/>
                   			</td>
						</tr>
						<tr>
							...
							<td>
								<select class="form-control" name="type" >
		           					<option value="0" ${area.type==0?"selected":""}>省份</option>
		           					<option value="1" ${area.type==1?"selected":""}>地市州</option>
		           					<option value="2" ${area.type==2?"selected":""}>区县</option>
	           					</select>
                   			</td>
						</tr>
						<tr>
							...
							<td>
								<textarea id="text-area" class="form-control" rows="5" name="remark">${area.remark}</textarea>
	...
	layui.use(['jquery','layer','laytpl'], function(){
		$("#editAreaForm input,select").on("change",function(){
		...
		$('#sub').on('click', function() {
			errors=[];
			if( !$('#editAreaForm').isValid({},conf, true)) {
				isForm=false;
				...
				postFormData("editAreaForm",true,editArea);
		    }
