...
<body >
	...
	<div style="padding: 10px;" class="form-inline">
		  ...
		  <!-- <div class="panel-body"> -->
			  <table class="table" style="margin:0">
					...
					<tr style="background-color: #f9f9f9;">
						<td  width=10%>机构名称：</td>
						<td width=40%  id="orgName">${org.name}</td>
						<td width=10%>机构类型：</td>
						<td width=40% id="orgType">${org.typeName}</td>
					</tr>
					<tr style="background-color: #f9f9f9;">
						<td>描述：</td>
						<td id="orgDesc">${org.summary}</td>
						<td></td>
						<td></td>
					</tr>
					</tbody>
		...
		<div  style="padding-top: 5px;">
            ...
            <a class="btn btn-success btn-sm" onclick="addOrgUser(${orgId})" style="margin-bottom: 10px;">
                <i class="fa fa-plus">添加用户</i>
            </a>
          
            <a class="btn btn-danger btn-sm" onclick="delOrgUser()" id="del" style="margin-bottom: 10px;">
            	<i class="fa fa-trash-o">移除用户</i>
            </a>
	                    
		    <form name="searchOrgUserForm" class="form-inline" id="searchOrgUserForm" action="${webroot}org/orgUsers.do" style="margin-bottom: 10px;float: right;" method="post" onsubmit="return false">
...
<script type="text/javascript">
	layui.use(['jquery','layer','laytpl'], function(){
		...
		postFormData("searchOrgUserForm",true,searchResult);
	...
	function addOrgUser(id){
		...
		var index=layer.open({
		  ...
		  area: ['800px'],
		  scrollbar: false,
		  content: "${webroot}org/selectOrgUser.html?oId="+id,
		  end: function(){
			  loadPage();
		  }
		});
	...
	*/
	function searchResult(data){
		...
		if(data.success){
    		...
    		if(list && list.length > 0){
        		...
        		var pageHtml = createOrgUserPage(page, "searchOrgUserForm");
	...
	function createOrgUserPage(page,searchFormId){
	...
	function delOrgUser(){
		...
		if(!ids){
			...
			layer.confirm('确定要从该机构中移除选中的用户吗？', {icon: 3, title:'系统提示',
			  btn: ['确定','取消']
			}, function(){
				var url="${webroot}org/delOrgUsers.do";
				params={"ids":ids,"orgId":orgId};
				postData(url, params, true, delOrgUserResult); 
	...
	**回调函数
	*/
	function delOrgUserResult(data){
		var laytpl = layui.laytpl;
		var layer = layui.layer;
		var message=data.message;
