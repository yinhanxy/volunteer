...
<body class="v-page-body">
	<div class="page-container">
		<div class="panel">	
				<form id="editOrgForm" action="${webroot}org/saveOrg.do">
					<div>
					<table class="table table-bordered dataTable no-footer">
						<tr class="row">
							...
							<td class="col-xs-4">
									...
									<input type="text"  class="form-control" name="name"   placeholder="机构名称(由最少两个汉字组成且不能包含其他字符)" value="${org.name}"
									data-validation="required custom" data-validation-regexp="^[\u4e00-\u9fa5]{2,50}$"
							      	data-validation-error-msg-required="此项为必填项"
		                   			data-validation-error-msg-custom="由最少两个汉字组成且不能包含其他字符" 
		                   			data-validation-error-msg-container="#nError"/>
                   			</td>
                   			<%-- <td class="col-xs-2"><label for="inputEmail1">机构代码</label></td>
							<td class="col-xs-4">
									<input type="text"  class="form-control" name="code"   placeholder="机构代码" value="${org.code}"
									data-validation="custom" data-validation-regexp="^[0-9]{3,20}|[0-9]{0}$"
							...
							<td class="col-xs-4">
								<div class="input-group" ${empty org.parentName?'':'onclick="selectOrg()"'} >
									<input type="hidden" name="parentId"  id="parentId" value="${org.pId}"/>
									<input type="text"  class="form-control" name="parentName" id="parentName"  readonly="readonly" placeholder="上级机构名称" value="${empty org.parentName?'无':org.parentName}"
						...
						<tr class="row">
							...
							<label class="radio-inline">
							  	<input type="radio" name="type" value="1" style="margin-top: 9px;" ${org.type==1? 'checked':''}/>管理机构
							</label>
							<label class="radio-inline">
								<input type="radio" name="type" value="2" style="margin-top: 9px;" ${org.type==2? 'checked':''}/>业务机构
							</label>
                   			</td>
							...
							<td>
						    	...
						    	data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1">
		           					<option value="0" ${org.grade==0? "selected":""}>请选择</option>
		           					<option value="1" ${org.grade==1? "selected":""}>一级</option>
		           					<option value="2" ${org.grade==2? "selected":""}>二级</option>
		           					<option value="3" ${org.grade==3? "selected":""}>三级</option>
		           					<option value="4" ${org.grade==4? "selected":""}>四级</option>
	           					</select>
                   			</td>
						</tr>
						<tr class="row">
							...
							<td class="col-xs-4">
								<div class="input-group" onclick="selectArea()">
									...
									<input type="text"  class="form-control" name="areaName"  id="areaName" readonly="readonly" placeholder="区域名称" value="${org.areaName}"
									data-validation="required" data-validation-error-msg-required="此项为必填项" data-validation-error-msg-container="#nError"/>
									<span class="input-group-btn">
						...
						<tr class="row">
							...
							<td class="col-xs-4">
									<input type="text"  class="form-control" name="contact"   placeholder="联系人" value="${org.contact}"
							...
							<td class="col-xs-4">
									<input type="text"  class="form-control" name="mobile"   placeholder="联系电话" value="${org.mobile}"
						...
						<tr class="row">
							...
							<td class="col-xs-4" colspan="3">
									<input type="text"  class="form-control" name="email"   placeholder="邮箱地址" value="${org.email}" 
						...
						<tr class="row">
							...
							<td class="col-xs-4" colspan="3">
									<input type="text"  class="form-control" name="address"   placeholder="机构地址" value="${org.address}"
									data-validation="required" data-validation-error-msg-required="此选项为必填" data-validation-error-msg-container="#nError1"/>
                   			</td>
                   		</tr>
						<tr class="row">
							...
							<td colspan="3">
								<textarea id="text-area" class="form-control" rows="5" name="summary">${org.summary}</textarea>
	...
	layui.use(['jquery','layer','laytpl'], function(){
		...
		$("#editOrgForm input,select").on("change",function(){
		...
		$("#editOrgForm input[name='type']").on("change",function(){
		...
		$('#sub').on('click', function() {
			...
			if( !$('#editOrgForm').isValid({},conf, true)) {
				isForm=false;
				...
				postFormData("editOrgForm",true,editOrg);
		    }
