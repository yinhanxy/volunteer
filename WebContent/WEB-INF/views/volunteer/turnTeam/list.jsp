		        ...
		        <div class="page-title list-page">
		            <div class="title-env">
		                <ol class="breadcrumb bc-1" style="margin: 0px;">
		                    ...
		                    <li class="active">
		                        <a  href="${webroot}turnTeam/list.html"><strong>转队管理</strong></a>
		                    </li>
		                </ol>
		            </div>
		        </div>
		        ...
		        <div class="table-panel panel">
		            ...
		            <div class="panel-heading" style="padding-bottom: 0px;">
		           			<div class="btn-toolbar" style="margin-left: 0px;">
			           			<form class="form-inline" id="searchForm" action="${webroot}turnTeam/volunteerList.do" method="POST">
		                    ...
		                    <script id="templateData" type="text/html">
			                        ...
			                        <tr role="row"
			                            ...
			                            <td>
			                                <div>
			                                   <span>
													<shiro:hasPermission name="/turnTeam/view.html">
														<a href="javascript:void(0)" onclick="viewTurnTeam({{item.id}})">
													</shiro:hasPermission>
															{{item.realName}}
											   		<shiro:hasPermission name="/turnTeam/view.html">	
														</a>
										...
										<td>
										 ...
										 <td>
			                                <div>
			                                   <span>{{item.sdealUser}}</span>
			                                </div>
			                            </td>
										<td>
			                                <div>
			                                   <span>{{item.turnTeamResult}}</span>
			                                </div>
			                            </td>
										<td>
			                                <div>
											<span class="btn-group btn-group-sm btn-group-xs">
												<shiro:hasPermission name="/turnTeam/deal.html">
 									{{#  if(null==item.sourceResult || 0==item.sourceResult){ }}
    									<a class="btn btn-info btn-icon icon-left btn-one-last" style="margin-right:5px;" title="转队处理" onclick="dealTurnTeam({{item.id}})"><i class="fa fa-pencil-square-o">转队处理</i></a>
									{{#  } }}
									{{#  if(null!=item.sourceResult && 0!=item.sourceResult && 2!=item.sourceResult && (item.targetResult == null || item.targetResult == 0)){ }}
    									<a class="btn btn-info btn-icon icon-left btn-one-last" style="margin-right:5px;" title="转队处理" onclick="dealTurnTeam({{item.id}})"><i class="fa fa-pencil-square-o">转队处理</i></a>
									{{#  } }}
												</shiro:hasPermission>
                                            </span>
	...
	*/
	...
	function viewTurnTeam(id){
		...
		var index=layer.open({
		  ...
		  maxmin: true,
		  scrollbar: false,
		  content: ["${webroot}turnTeam/view.html?id="+id],
	...
	function dealTurnTeam(id){
		...
		var index=layer.open({
		  ...
		  maxmin: true,
		  scrollbar: true,
		  content: ["${webroot}turnTeam/deal.html?id="+id],
		  btn: ['<i class="fa fa-check-circle"></i>通过','<i class="fa fa-ban"></i>不通过','<i class="fa fa-remove"></i>取消'],
		  yes : function(index, layero) {
				var iframeWin = top.window[layero.find('iframe')[0]['name']]; 
				iframeWin.passTurn();
				return false;
